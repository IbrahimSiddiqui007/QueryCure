...
void DiagnosisAnalysis::runEPButtonClicked(wxCommandEvent& evt) {
	std::string SqlString = (std::string)inputCtrl->GetValue();

	SAString SqL = SqlString.c_str();
	wxString output;
	//m_p.GetCommand().setCommandText("Explain Plan For "+SqL);
	//m_p.GetCommand().Execute();

	//m_p.GetCommand().setCommandText("SELECT plan_table_output FROM TABLE(DBMS_XPLAN.DISPLAY())");
	//m_p.GetCommand().Execute();

 //  while (m_p.GetCommand().FetchNext())
 //  {
	//   std::cout << (std::string)m_p.GetCommand()[1].asString() << std::endl;;
 //  }
   m_p.GetCommand().setCommandText("Explain Plan For " + SqL);
   m_p.GetCommand().Execute();
   ...
   while (m_p.GetCommand().FetchNext()) {
	   SAString fieldStr = m_p.GetCommand()[1].asString();
	   wxString wxStr = wxString::FromUTF8(fieldStr.GetMultiByteChars());
	   output += wxStr + wxT("\n");
   }

   outputCtrl->SetValue(output);
...
void DiagnosisAnalysis::connectButtonClicked(wxCommandEvent& evt)
{
	...
	statsChoice->Append("Tables");
	statsChoice->Append("Indexes");
	statsChoice->Append("Materialized views");
	statsChoice->Append("Table Partitions");
	statsChoice->Append("Columns of tables");
	statsChoice->Append("Clusters");
